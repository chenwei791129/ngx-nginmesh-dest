 ...
 *
 */
static ngx_command_t ngx_http_mesh_commands[] = {
...
/* The module context. */
static ngx_http_module_t ngx_http_mesh_module_ctx = {
    ngx_http_ngin_add_variables, /* preconfiguration */
...
/* Module definition. */
ngx_module_t ngx_http_nginmesh_dest_module = {
    NGX_MODULE_V1,
    &ngx_http_mesh_module_ctx, /* module context */
    ngx_http_mesh_commands, /* module directives */
...
static ngx_http_variable_t  ngx_http_nginmesh_vars[] = {

    { ngx_string("nginmesh_dest"), NULL,
      ngx_http_nginmesh_dest_variable, 0, 0, 0 },
...
static ngx_int_t ngx_http_nginmesh_handler(ngx_http_request_t *s)
{
	...
	ngx_log_debug(NGX_LOG_DEBUG_HTTP,  c->log, 0,"nginmesh http handler invoked");

    meshcf = ngx_http_get_module_srv_conf(s, ngx_http_nginmesh_dest_module);

    if (!meshcf->enabled) {
        ngx_log_debug(NGX_LOG_DEBUG_HTTP, c->log, 0, "nginmesh not enabled, declined");
        return NGX_DECLINED;
    }

    // SOCK_DGRAM mean UDP, SOCK_STREAM mean TCP
    if (c->type != SOCK_STREAM) {
        ngx_log_debug(NGX_LOG_DEBUG_HTTP, c->log, 0, "nginmesh not tcp, declined");
        return NGX_DECLINED;
    }

    // if (c->buffer == NULL) {
    //     ngx_log_debug(NGX_LOG_DEBUG_HTTP, c->log, 0, "nginmesh no buffer, again");
    //     return NGX_AGAIN;
    // }
    ...
    if (ctx == NULL) {
         ngx_log_debug(NGX_LOG_DEBUG_HTTP, c->log, 0, "nginmesh creating new context");
        ctx = ngx_pcalloc(c->pool, sizeof(ngx_http_nginmesh_ctx_t));
        if (ctx == NULL) {
            return NGX_ERROR;
        }

        ngx_http_set_ctx(s, ctx, ngx_http_nginmesh_dest_module);
  ...
  //  len = ngx_sock_ntop(c->sockaddr, sizeof(struct sockaddr_in),text,c->socklen,1);
   ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "module original ip address: %*s",c->addr_text.len,c->addr_text.data);
    ...
    if(getsockopt ( c->fd, SOL_IP, SO_ORIGINAL_DST, &org_src_addr,&org_src_addr_len) == -1) {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log,0, "ip address length %d",org_src_addr_len);

        if(org_src_addr.ss_family == AF_INET )  {
           ngx_log_debug(NGX_LOG_DEBUG_HTTP, c->log, 0, "address is is INET format");
           struct sockaddr_in *addr_in = (struct sockaddr_in *)&org_src_addr;
           char *s = inet_ntoa(addr_in->sin_addr);
           int port = ntohs(addr_in->sin_port);
           ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log, 0, "founded dest ip address: %s",s);
           ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log,  0, "founded dest port address: %d",port);

           ngx_memzero(dest_text,30);
           sprintf(dest_text,"%s:%d",s,port);
           ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log,  0, "combined text: %s",dest_text);
           ...
           ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log,  0, "ctx dest  var %*s",ctx->dest.len,ctx->dest.data);


           
        } else {

           ngx_log_debug(NGX_LOG_DEBUG_HTTP, c->log,  0, "address is is not INET format");
           return NGX_DECLINED;
        }
    ...
    ngx_variable_value_t *v, uintptr_t data)
{
	...
	ngx_log_debug2(NGX_LOG_DEBUG_HTTP,  c->log, 0,"set var nginmesh_dest %*s",v->len,v->data);
...
static ngx_int_t ngx_http_ngin_add_variables(ngx_conf_t *cf)
{
    ...
    for (v = ngx_http_nginmesh_vars; v->name.len; v++) {
